...
function navigateTo(page: PageName) {
  window.dispatchEvent(new CustomEvent<PageName>("lp:navigate", { detail: page }));
}
...
function Shell({
  ...
  React.useEffect(() => {
    const handler = (event: Event) => {
      const customEvent = event as CustomEvent<PageName>;
...
function Card({ icon: Icon, title, body, onClick }: CardProps) {
  const content = (
    <>
      {Icon && (
  ...
  return (
    <div className="group rounded-[1.75rem] border border-[#c9a227]/26 bg-[linear-gradient(180deg,rgba(255,255,255,.055),rgba(255,255,255,.025))] p-6 text-left shadow-2xl shadow-black/20 transition hover:-translate-y-1 hover:border-[#c9a227]/55 hover:bg-white/[.065]">
      {content}
    </div>
  );
}
    ...
    <>
      ...
      >
        <div className="mt-9 flex flex-wrap gap-4">
          ...
          <button
            onClick={() => setCurrentPage("Capability Matrix")}
            type="button"
      ...
      <section className="mx-auto max-w-7xl px-5 py-6 lg:px-8">
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          ...
          />
          <Card
            onClick={() => setCurrentPage("Entropy Diagnostic")}
            icon={Gauge}
          ...
          />
          <Card
            onClick={() => setCurrentPage("Office of the Administrator")}
            icon={Building2}
...
function CapabilityMatrix({ openInquiry }: PageWithInquiryProps) {
  const rows = [
    [
      "Research & Framing",
      "Problem definition",
...
function EntropyDiagnostic({ openInquiry }: PageWithInquiryProps) {
  return (
    <>
      <Hero
      ...
      <section className="mx-auto max-w-7xl px-5 py-6 lg:px-8">
        ...
        <div className="grid gap-5 md:grid-cols-4">
          <Card
            ...
            body="Is the problem legible, or buried in noise?"
          />
          <Card
            icon={ClipboardList}
            title="Assumption Load"
            body="How much depends on unstated premises?"
          />
          <Card
            icon={ShieldCheck}
            title="Claim Stability"
            body="Which claims are durable, and which are weak?"
          />
          <Card
            icon={Workflow}
            title="Decision Friction"
            body="Where is confusion slowing action or judgment?"
          />
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-6 lg:px-8">
        ...
        <div className="grid gap-5 md:grid-cols-4">
          <Card
            icon={Target}
            title="Entropy Scorecard"
            body="A comprehensive view of ambiguity and risk across key dimensions."
          />
          <Card
            icon={ShieldCheck}
            title="Risk Notes"
            body="Concise notes on vulnerabilities and decision risks."
          />
          <Card
            icon={MessageSquare}
            title="Clarifying Questions"
            body="Targeted questions that surface hidden gaps and assumptions."
          />
          <Card
            icon={Workflow}
            title="Recommended Interventions"
            body="Practical actions to reduce entropy and improve decision quality."
          />
        </div>
      </section>
...
function Engagement({ openInquiry }: PageWithInquiryProps) {
  return (
    <>
      <Hero
      ...
      <section className="mx-auto max-w-7xl px-5 py-6 lg:px-8">
        ...
        <div className="grid gap-5 md:grid-cols-4">
          <Card
            icon={MessageSquare}
            title="Briefing"
            body="A focused framing session for a live issue, decision, or strategic question."
          />
          <Card
            icon={Gauge}
            title="Diagnostic Sprint"
            body="A short, structured review that identifies ambiguity, risks, and intervention paths."
          />
          <Card
            icon={Network}
            title="Architecture Design"
            body="A deeper engagement for systems, workflows, protocols, and operating structure."
          />
          <Card
            icon={Users}
            title="Advisory Partnership"
            body="Ongoing support for decision quality, governance, and disciplined execution."
          />
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 py-6 lg:px-8">
        ...
        <div className="grid gap-5 md:grid-cols-4">
          <Card
            icon={Target}
            title="Poorly Framed"
            body="The issue is important but poorly framed."
          />
          <Card
            icon={Gauge}
            title="Ambiguity or Drift"
            body="The team is dealing with ambiguity or drift."
          />
          <Card
            icon={Network}
            title="System Redesign"
            body="A system, workflow, or protocol needs redesign."
          />
          <Card
            icon={ShieldCheck}
            title="Sharper Briefs"
            body="Decision-makers need a sharper brief, not more noise."
          />
        </div>
      </section>
...
function Administrator({ openInquiry }: PageWithInquiryProps) {
  return (
    <>
      <Hero
      ...
      <section className="mx-auto max-w-7xl px-5 py-6 lg:px-8">
        ...
        <div className="grid gap-5 md:grid-cols-4">
          <Card
            ...
            body="Maintain the operating principles and quality thresholds of the protocol."
          />
          <Card
            icon={RefreshCcw}
            title="Continuity Management"
            body="Keep work coherent across engagements, documents, and evolving systems."
          />
          <Card
            icon={ShieldCheck}
            title="Governance Review"
            body="Ensure decision structures remain transparent and defensible."
          />
          <Card
            icon={Mail}
            title="Administrative Inquiry"
            body="Provide a formal channel for requests, records, and administrative coordination."
          />
        </div>
      </section>
        ...
        <div>
          ...
          <div className="overflow-hidden rounded-[2rem] border border-[#c9a227]/30 bg-[#0d1016]">
            {[
              ...
              >
                <CheckCircle2
                  className="mr-3 inline text-[#c9a227]"
                  size={18}
                />{" "}
                {item}
              </div>
            ...
            <div className="bg-[#c9a227]/10 p-5 text-sm text-[#f0d37c]">
              Credibility is built through restraint, transparency, and
              consistency.
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-6 lg:px-8">
        ...
        <div className="grid gap-5 md:grid-cols-4">
          <Card
            ...
            body="Formal requests related to the protocol."
          />
          <Card
            icon={MessageSquare}
            title="Governance Questions"
            body="Questions about standards and operating logic."
          />
          <Card
            icon={Users}
            title="Coordination Notes"
            body="Coordination across documents and engagements."
          />
          <Card
            icon={FileText}
            title="Record Inquiries"
            body="Requests involving records or continuity."
          />
        </div>
      </section>
...
export default function LipeProtocolSite() {
  ...
  const content = useMemo(() => {
    switch (currentPage) {
      ...
      default:
        return (
          <Home setCurrentPage={setCurrentPage} openInquiry={openInquiry} />
        );
    }
  }, [currentPage]);
